...
class Blip2OPTInstruct(Blip2OPT):
    ...
    def forward(self, samples):
        ...
        clue_mask = input_tokens.input_ids != self.opt_tokenizer.pad_token_id
        clue_mask_outputshape = torch.zeros_like(output_tokens.input_ids, dtype=torch.bool)
        clue_mask_outputshape[:,:clue_mask.shape[1]] = clue_mask

        # fill right-padded tokens to -100
        targets = output_tokens.input_ids.masked_fill(
            output_tokens.input_ids == self.opt_tokenizer.pad_token_id, -100
        )

        # fill clue tokens to -100
        targets = targets.masked_fill(
            clue_mask_outputshape, -100
        )
        ...
        inputs_embeds = torch.cat([inputs_opt, inputs_embeds], dim=1)
        attention_mask = torch.cat([atts_opt, output_tokens.attention_mask], dim=1)
